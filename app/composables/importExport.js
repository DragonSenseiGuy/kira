 ...
 * @description Import/export system for Kira. Exports conversations,
...
export const EXPORT_FORMAT = "kira-export";
// Archives produced by legacy upstream versions remain importable.
export const LEGACY_EXPORT_FORMATS = ["libre-assistant-export"];
export const EXPORT_FORMAT_VERSION = 1;

export function isSupportedExportFormat(format) {
  return format === EXPORT_FORMAT || LEGACY_EXPORT_FORMATS.includes(format);
}
...
export function validateManifest(manifest) {
  ...
  if (!isSupportedExportFormat(manifest.format)) {
...
/**
 * Converts an OpenWebUI chat export into a native Kira conversation.
 */
...
export function parseImportArchive(buffer) {
  ...
  if (manifest && isSupportedExportFormat(manifest.format)) {
  ...
  // Fallback: if no native manifest was found, look for native chat files by name.
  if (!manifest || !isSupportedExportFormat(manifest.format)) {
    console.log("[importExport] No native manifest found; scanning for native chat files");
 ...
 */
export function generateExportFilename(prefix = "kira-export") {
